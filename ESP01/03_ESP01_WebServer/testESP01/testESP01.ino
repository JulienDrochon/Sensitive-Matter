...
int builtinLED = 13;                      // assign a variable named "redLED" with an integer value 12, it refers to the pin which the red LED is connected on.
...
String ssid = "SFR-aea0";
String pwd = "QITEL7AX2FEV";

void setup()
{
  pinMode(LED_BUILTIN, OUTPUT);  // set the pin number 12 as an output pin.
  pinMode(blueLED, OUTPUT); // set the pin number 11 as an output pin.

  digitalWrite(LED_BUILTIN, HIGH);               // turn the LED_BUILTIN off at the beginning of the program.
...
void loop() // our main program, some fun are about to start)
{
  if (esp8266.available()) // if there's any data received and stored in the serial receive buffer, go and excute the if-condition body. If not, dont excute the if-condition body at all.
  {
    ...
    {
      delay(50);                                   // wait 1 second to fill up the buffer with the data.
